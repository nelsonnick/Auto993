package com.wts.entity;

// 家属
public class PersonJS {
    private String ldnl;//劳动能力
    private String jsgmsfhm;//家属公民身份号码
    private String bz;//备注
    private String hzgx;//户主关系
    private String hzgrxm;//户主个人姓名
    private String hzgmsfhm;//户主公民身份号码
    private String jslxdh;//家属联系电话
    private String jsjkzk;//家属健康状况
    private String jscsrq;//家属出生日期
    private String jsxm;//家属姓名
    private String jsryzt;//家属人员状态
    private String jswhcd;//家属文化程度
    private String txdz;//通讯地址
    private String jsxh;//家属序号
    private String jsysr;//家属月收入
    private String jsgzdw;//家属工作单位
    private String jyyy;//就业意愿
    private String jsxb;//家属性别

    public String getHzgrxm() {
        return hzgrxm;
    }

    public void setHzgrxm(String hzgrxm) {
        this.hzgrxm = hzgrxm;
    }

    public String getHzgmsfhm() {
        return hzgmsfhm;
    }

    public void setHzgmsfhm(String hzgmsfhm) {
        this.hzgmsfhm = hzgmsfhm;
    }

    public String getLdnl() {
        return ldnl;
    }

    public void setLdnl(String ldnl) {
        this.ldnl = ldnl;
    }

    public String getJsgmsfhm() {
        return jsgmsfhm;
    }

    public void setJsgmsfhm(String jsgmsfhm) {
        this.jsgmsfhm = jsgmsfhm;
    }

    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz;
    }

    public String getHzgx() {
        return hzgx;
    }

    public void setHzgx(String hzgx) {
        this.hzgx = hzgx;
    }

    public String getJslxdh() {
        return jslxdh;
    }

    public void setJslxdh(String jslxdh) {
        this.jslxdh = jslxdh;
    }

    public String getJsjkzk() {
        return jsjkzk;
    }

    public void setJsjkzk(String jsjkzk) {
        this.jsjkzk = jsjkzk;
    }

    public String getJscsrq() {
        return jscsrq;
    }

    public void setJscsrq(String jscsrq) {
        this.jscsrq = jscsrq;
    }

    public String getJsxm() {
        return jsxm;
    }

    public void setJsxm(String jsxm) {
        this.jsxm = jsxm;
    }

    public String getJsryzt() {
        return jsryzt;
    }

    public void setJsryzt(String jsryzt) {
        this.jsryzt = jsryzt;
    }

    public String getJswhcd() {
        return jswhcd;
    }

    public void setJswhcd(String jswhcd) {
        this.jswhcd = jswhcd;
    }

    public String getTxdz() {
        return txdz;
    }

    public void setTxdz(String txdz) {
        this.txdz = txdz;
    }

    public String getJsxh() {
        return jsxh;
    }

    public void setJsxh(String jsxh) {
        this.jsxh = jsxh;
    }

    public String getJsysr() {
        return jsysr;
    }

    public void setJsysr(String jsysr) {
        this.jsysr = jsysr;
    }

    public String getJsgzdw() {
        return jsgzdw;
    }

    public void setJsgzdw(String jsgzdw) {
        this.jsgzdw = jsgzdw;
    }

    public String getJyyy() {
        return jyyy;
    }

    public void setJyyy(String jyyy) {
        this.jyyy = jyyy;
    }

    public String getJsxb() {
        return jsxb;
    }

    public void setJsxb(String jsxb) {
        this.jsxb = jsxb;
    }
}
