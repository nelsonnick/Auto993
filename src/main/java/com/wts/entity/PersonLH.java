package com.wts.entity;

// 灵活就业
public class PersonLH {
  private String whcd;//文化程度
  private String cjrmc;//创建人名称
  private String sqmc;//社区名称
  private String djlsh;//登记流水号
  private String yanglaobt;//养老补贴
  private String wtgyy;//未通过原因
  private String zzny;//终止年月
  private String spzt;//审批状态 03已通过
  private String knrybh;//困难人员编号
  private String tcrq;//退出日期
  private String btje;//补贴金额
  private String qsny;//起始年月
  private String cjrq;//创建日期
  private String yiliaobt;//医疗补贴
  private String cjjgmc;//创建机构名称
  private String jydjbh;//XX登记编号
  private String gmsfhm;//公民身份号码
  private String grxm;//个人姓名
  private String lxdh;//联系电话
  private String lhjycszy;//灵活就业从事职业
  private String xb;//性别 1男2女
  private String sfyba;//是否已备案 0未备案1已备案
  private String grbh;//个人编号

  public String getWhcd() {
    return whcd;
  }

  public void setWhcd(String whcd) {
    this.whcd = whcd;
  }

  public String getCjrmc() {
    return cjrmc;
  }

  public void setCjrmc(String cjrmc) {
    this.cjrmc = cjrmc;
  }

  public String getSqmc() {
    return sqmc;
  }

  public void setSqmc(String sqmc) {
    this.sqmc = sqmc;
  }

  public String getDjlsh() {
    return djlsh;
  }

  public void setDjlsh(String djlsh) {
    this.djlsh = djlsh;
  }

  public String getYanglaobt() {
    return yanglaobt;
  }

  public void setYanglaobt(String yanglaobt) {
    this.yanglaobt = yanglaobt;
  }

  public String getWtgyy() {
    return wtgyy;
  }

  public void setWtgyy(String wtgyy) {
    this.wtgyy = wtgyy;
  }

  public String getZzny() {
    return zzny;
  }

  public void setZzny(String zzny) {
    this.zzny = zzny;
  }

  public String getSpzt() {
    return spzt;
  }

  public void setSpzt(String spzt) {
    this.spzt = spzt;
  }

  public String getKnrybh() {
    return knrybh;
  }

  public void setKnrybh(String knrybh) {
    this.knrybh = knrybh;
  }

  public String getTcrq() {
    return tcrq;
  }

  public void setTcrq(String tcrq) {
    this.tcrq = tcrq;
  }

  public String getBtje() {
    return btje;
  }

  public void setBtje(String btje) {
    this.btje = btje;
  }

  public String getQsny() {
    return qsny;
  }

  public void setQsny(String qsny) {
    this.qsny = qsny;
  }

  public String getCjrq() {
    return cjrq;
  }

  public void setCjrq(String cjrq) {
    this.cjrq = cjrq;
  }

  public String getYiliaobt() {
    return yiliaobt;
  }

  public void setYiliaobt(String yiliaobt) {
    this.yiliaobt = yiliaobt;
  }

  public String getCjjgmc() {
    return cjjgmc;
  }

  public void setCjjgmc(String cjjgmc) {
    this.cjjgmc = cjjgmc;
  }

  public String getJydjbh() {
    return jydjbh;
  }

  public void setJydjbh(String jydjbh) {
    this.jydjbh = jydjbh;
  }

  public String getGmsfhm() {
    return gmsfhm;
  }

  public void setGmsfhm(String gmsfhm) {
    this.gmsfhm = gmsfhm;
  }

  public String getGrxm() {
    return grxm;
  }

  public void setGrxm(String grxm) {
    this.grxm = grxm;
  }

  public String getLxdh() {
    return lxdh;
  }

  public void setLxdh(String lxdh) {
    this.lxdh = lxdh;
  }

  public String getLhjycszy() {
    return lhjycszy;
  }

  public void setLhjycszy(String lhjycszy) {
    this.lhjycszy = lhjycszy;
  }

  public String getXb() {
    return xb;
  }

  public void setXb(String xb) {
    this.xb = xb;
  }

  public String getSfyba() {
    return sfyba;
  }

  public void setSfyba(String sfyba) {
    this.sfyba = sfyba;
  }

  public String getGrbh() {
    return grbh;
  }

  public void setGrbh(String grbh) {
    this.grbh = grbh;
  }
}
